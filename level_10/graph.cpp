...
Graph::Graph(int V) : V(V), maxSCCSize(0) , currentEdges(0)  {
...
void Graph::DFSUtil(int v, vector<bool>& visited, stringstream& answer ,vector<int>& currentSCC) {
    visited[v] = true;
    answer << v + 1 << " ";
    currentSCC.push_back(v);
    for (int i : adj[v])
        if (!visited[i])
            DFSUtil(i, visited, answer , currentSCC );
}

    int Graph:: getMaxSCCSize()  { return maxSCCSize; }

    int Graph:: getVertexCount() { return V; }
...
string Graph::printSCCs() {
    ...
    maxSCCSize = 0; // Reset the maximum SCC size
    ...
    while (!Deque.empty()) {
        ...
        if (!visited[v]) {
            ...
            answer << "Strongly Connected Component " << ++componentCount << ":\n";
             // Temporary vector to store the current SCC
            vector<int> currentSCC;
            
            gr.DFSUtil(v, visited, answer ,currentSCC);

              // Update the max SCC size
            maxSCCSize = std::max(maxSCCSize, static_cast<int>(currentSCC.size()));
            answer << "\n";
        }
    }

...
// POSIX condition variable for SCC tracking
std::mutex condMutex;
std::condition_variable condVar;
bool sccState = false; // Indicates whether the graph is above 50% connectivity
bool prevState = false; // Previous state for change detection


// Function to monitor SCC state
void monitorSCC() {
    std::unique_lock<std::mutex> lock(condMutex);

    while (true) {
        condVar.wait(lock, [] { return sccState != prevState; });

        if (sccState) {
            std::cout << "At least 50% of the graph belongs to the same SCC." << std::endl;
        } else {
            std::cout << "Less than 50% of the graph belongs to the same SCC." << std::endl;
        }
    ...
    while (true) {
        ...
        if (receivedMessage.find("Newgraph") == 0) {
        ...
        }  else if (receivedMessage.find("Kosaraju") == 0) {
            {
                ...
                // Update SCC state
                int maxSCC = globalGraph.getMaxSCCSize();
                sccState = (maxSCC >= globalGraph.getVertexCount() / 2);
               

            }
             condVar.notify_one(); // Notify the monitoring thread
            std::cout << "Server: " << response;
            send(clientSocket, response.c_str(), response.size(), 0);
...
int main() {
    ...
    std::thread monitorThread(monitorSCC);
